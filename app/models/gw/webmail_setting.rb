...
class Gw::WebmailSetting < ActiveRecord::Base
  ...
  def set_value(params)
    ...
    when :label_color_field
      if !params[:name].blank? && !params[:color].blank?
        self.value = {:id => params[:label_id], :name => params[:name], :color => params[:color], :state => params[:state]}.to_json
      else
        self.value = ''
      end
    else
  ...
  def self.switch_user_count
    self.count(:conditions => ["user_id = ? AND name LIKE 'switch_user%'", Core.user.id])
  end
  ...
  def self.load_switch_users
    ...
    confs.each do |conf|
      if user = Sys::User.find_by_account(conf[:account])
  ...
  def self.load_label_confs
    ...
    (1..9).each do |idx|
      ...
      if !value.blank?
        ...
        confs << conf if conf[:state] == 'enabled' && !conf[:name].blank?
      end
    end
    confs
  end
